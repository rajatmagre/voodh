...
DB::enableQueryLog();
class ApiControllerKamlesh extends Controller
{ 
    ...
    public function new_registration(Request $request)
    {
      if ($request->isMethod('post')) {
          ...
          if(!empty($checkMobile)){
            return response()->json(['msg' => 'Mobile number already exists.','status' => 0]);
          }
          $checkEmail  = DB::table('tbl_user_registration')->where(['email' => $request->email])->first();
          if(!empty($checkEmail)){
              return response()->json(['msg' => 'Email already exists.','status' => 0]);
          }
          ...
          if(!empty($id))
          {
              ...
              if(!empty($res)) {
                ...
                $UserData = array(
                  ...
                  return response()->json(['result' => $UserData,'status' => 1,"msg"=>"success"]);
              } else {
    ...
    public function veryfyOtp(Request $request) {
      if ($request->isMethod('post')) {
        ...
        if (empty($lang_type)) {
          ...
          /************update opt***********/
          if(!empty($res)) {
              $arr = array('verified_otp' => 'yes', 'last_login'=>date('Y-m-d H:i:s'));
              ...
              if ($lang_type == 'English') {
                ...
                $success_msg = $this->get_hindi_text($lang_type,"Login successfully.");
              }
  ...
  /*****************Resend opt api*****************/
          ...
          /************update opt***********/
            if(!empty($res)) {
              if ($res->verified_otp == 'yes') {
                ...
                // });
                $arr = array('otp' => NULL,'last_login'=>date('Y-m-d H:i:s'));
    ...
    public function get_profile(Request $request) {
      if ($request->isMethod('post')) {
          ...
          if(!empty($res)) {
            ...
            return response()->json(['msg' =>"Record Not Found",'status' => 0]);
          }
      }
    }
    // **************** Update Profile ************//
    public function update_profile(Request $request) {
      if ($request->isMethod('post')) {
          ...
          return response()->json(['result' => $UserData,'msg'=>"Profile updated successfully.",'status' => 1]);
      } else {
          return response()->json(['msg' =>"Something went worng.",'status' => 0]);
      }
    }
    ...
    public function blood_update_profile(Request $request) {
        if ($request->isMethod('post')) {
            ...
            // $blood_permanent_distance = $request->blood_permanent_distance;
            ...
            $newData = array(
                        ...
                        'aadhar_no'               =>  $aadhar_no,
                        // 'blood_permanent_distance'=>  $blood_permanent_distance,
            ...
            $UserData = array(
                        ...
                        'current_distance'  =>  isset($res->blood_current_distance)?$res->blood_current_distance:'',
    ...
    public function blood_donetion_list(Request $request) {
      ...
      if ($request->isMethod('post')) {
          ...
          $res = BloodRequirement::where(['tbl_blood_requirement.user_id' => $user_id, 'tbl_blood_requirement.deleted' => '0'])->orderBy('req_id','DESC')->with('user_details')->get();
          ...
          if ( !empty($res) && count($res) > 0) {
            ...
            foreach ($res as $val) {
              ...
              if ($lang_type == 'English') {
                ...
                $where_address   = $val->where_address;
                $status          = $val->status;
                ...
                $where_address  = $this->get_hindi_text($lang_type, $val->where_address);
                $status         = $this->get_hindi_text($lang_type, $val->status);
                $first_name     = $this->get_hindi_text($lang_type, $val->user_details->first_name);
                $last_name      = $this->get_hindi_text($lang_type, $val->user_details->last_name);
              ...
              $data[] = array(
                          ...
                          'requirement_date' =>  isset($val->requirement_date)?$val->requirement_date:'',
                        );
            }

            return response()->json(['result' => $data,'total_blood_units' => @$user_total_blood_units,'page_content' => @$page_content, 'status' => 1]);
    ...
    public function accident_track(Request $request) {
        ...
        if ($request->isMethod('post')) { 
          ...
          $AccidentTrackData   = AccidentTrack::where($where)->get();
    ...
    public function help_request_to_big_brother(Request $request) {
      if ($request->isMethod('post')) {
          ...
          if($id){
            ...
            if ($lang_type == 'English') {                
              $success_msg = "Help request successfully sent to big brother. ";
            }else{
              $success_msg = $this->get_hindi_text($lang_type, "Help request successfully sent to big brother.");
            }
            return response()->json(['msg' =>$success_msg,'status' => 1]);
          }else {
            if ($lang_type == 'English') {
